...
from yolox.utils import MeterBuffer, ModelEMA, all_reduce_norm, get_local_rank, get_model_info, get_rank, get_world_size
from yolox.utils import gpu_mem_usage, is_parallel, load_ckpt, occupy_mem, save_checkpoint, setup_logger, synchronize


class Trainer:
    def __init__(self, exp, args):
        # init function only defines some basic attr,
        # other attrs like model, optimizer are built in before_train methods.
        self.exp = exp
        self.args = args

        # training related attr
        self.max_epoch = exp.max_epoch
        self.freeze_epoch = 100
        ...
        setup_logger(self.file_name, distributed_rank=self.rank, filename="train.log", mode="a")
    ...
    def train_one_iter(self):
        ...
        #     if param.grad is not None and param.grad.norm() > 10000:
        #         print(f'Layer: {name} | Grad: {param.grad.norm()}')
    ...
    def before_train(self):
        ...
        self.max_iter = len(self.train_loader) # max_iter means iters per epoch
    ...
    def before_epoch(self):
        ...
        # if self.epoch + 1 <= self.freeze_epoch:
        #     logger.info("---> Freezing YOLO layers!")
        #     for name, param in self.model.named_parameters():
        #         if "backbone" in name:
        #             param.requires_grad = False
        # else:
        #     logger.info("---> Unfreezing YOLO layers!")
        #     for name, param in self.model.named_parameters():
        #         if 'vgg' not in name:
        ...
        # if self.is_distributed:
            # self.model.module.set_current_epoch(self.epoch+1)
        # else:
            # self.model.set_current_epoch(self.epoch+1)
    ...
    def after_epoch(self):
        ...
        # self.save_ckpt(ckpt_name='%03d'%(self.epoch+1))
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            progress_str = "ep:{}/{}, it:{}/{}".format(self.epoch + 1, self.max_epoch, self.iter + 1, self.max_iter)
            loss_meter = self.meter.get_filtered_meter("loss")
            loss_str = ", ".join(["{}:{:.3f}".format(k, v.latest) for k, v in loss_meter.items()])

            time_meter = self.meter.get_filtered_meter("time")
            time_str = ", ".join(["{}:{:.1f}s".format(k, v.avg) for k, v in time_meter.items()])

            logger.info("{}, {}, lr:{:.2e}".format(progress_str, loss_str, self.meter["lr"].latest) + (", {}".format(eta_str)))
