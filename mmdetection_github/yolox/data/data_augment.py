...
def augment_hsv_raw(img, hgain=5, sgain=30, vgain=30):
...
def augment_hsv(img, hgain=5, sgain=30, vgain=30):
    """HSV augmentation for uint8 BGR images (standard COCO / JPEG pipeline)."""
...
def preproc(img, input_size, swap=(2, 0, 1)):
    """Letterbox pad + resize for uint8 BGR; output float32 CHW in ~[0, 255] (YOLOX default)."""
    if len(img.shape) == 3:
...
class TrainTransform:
    """Augmentation + letterbox for standard RGB/BGR uint8 images (COCO JPEG)."""
    ...
    def __call__(self, image, targets, input_dim):
        ...
        boxes_o = xyxy2cxcywh(targets_o[:, :4].copy())
...
class ValTransform:
    """Val / test preprocessing for uint8 BGR images."""
...
def preproc_raw(img, input_size, swap=(2, 0, 1)):
    if img.shape[2] == 3:
        padded_img = np.ones((input_size[0], input_size[1], 3), dtype=np.float32) * 114
        r = min(input_size[0] / img.shape[0], input_size[1] / img.shape[1])
        # print(padded_img.shape)
    else:
        padded_img = np.ones((input_size[0]//2, input_size[1]//2, 4),dtype=np.float32) * 114
        r = 1
    # resized_img = img
...
class TrainTransformRaw:
    ...
    def __call__(self, image, targets, input_dim):
        ...
        if len(boxes) == 0:
            # print("0000")
            targets = np.zeros((self.max_labels, 5), dtype=np.float32)
            image, r_o = preproc_raw(image, input_dim)
            # print(image.max())
            return image, targets
        # print(image.max())
        image_o = image.copy()
        ...
        if random.random() < self.hsv_prob:
            augment_hsv_raw(image)
        # print(image.max())
        image_t, boxes = _mirror(image, boxes, self.flip_prob)
        height, width, _ = image_t.shape
        # print(image_t.max())
        image_t, r_ = preproc_raw(image_t, input_dim)
        ...
        if len(boxes_t) == 0:
            image_t, r_o = preproc_raw(image_o, input_dim)
            boxes_o *= r_o
            boxes_t = boxes_o
            labels_t = labels_o
...
class ValTransformRaw:
    ...
    def __call__(self, img, res, input_size):
        img, _ = preproc_raw(img, input_size, self.swap)
        # img /= 255.0
        if self.legacy:
            img = img[::-1, :, :].copy()
            img /= 255.0
