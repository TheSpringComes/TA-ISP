...
class MosaicDetection(Dataset):
    """Mosaic + mixup for uint8 BGR images (COCO JPEG / standard YOLOX)."""
...
class MosaicDetectionRaw(Dataset):
    ...
    def mixup(self, origin_img, origin_labels, input_dim):
        ...
        if len(img.shape) == 3:
            cp_img = np.ones((input_dim[0], input_dim[1], 3), dtype=np.float32) * 114
        else:
        ...
        origin_h, origin_w = cp_img.shape[:2]
        target_h, target_w = origin_img.shape[:2]
        padded_img = np.zeros((max(origin_h, target_h), max(origin_w, target_w), 3), dtype=np.float32)
        padded_img[:origin_h, :origin_w] = cp_img
