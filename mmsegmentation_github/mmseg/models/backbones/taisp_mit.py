...
class MultipleMaskISP_Conv(nn.Module):
    ...
    def forward(self, raw):
        ...
        raw_adj = (raw * dgain.reshape(B, C, 1, 1)).clamp(1e-6,1)
        raw_adj = self.attn(raw_adj).clamp(1e-6,1)

        # 4. mask logits and soft masks
        logits = self.mask_head(raw_adj)
        # masks = F.softmax(logits, dim=1)
        masks = F.gumbel_softmax(logits, dim=1, tau=self.tau)

        # 5. compute channel weights in [w_min, w_max]
        logits_pool = logits.reshape(B, self.num_masks, -1).mean(dim=-1)
        w_logit = self.weight_mlp(logits_pool)           
        w_norm = torch.sigmoid(w_logit)            
        w = self.w_min + (self.w_max - self.w_min) * w_norm
        w = w.reshape(B, self.num_masks, 1, 1, 1)
        # print(w)

        raw_adj = raw_adj.unsqueeze(1)
        raw_adj = raw_adj.pow(1/w).clamp(1e-6,1)
        out = (masks.unsqueeze(2) * raw_adj).sum(dim=1)
        return out


@MODELS.register_module()
class TAISP_MixVisionTransformer(BaseModule):
    ...
    def __init__(self,
        ...
        self.TAISP = MultipleMaskISP_Conv(weight_range=self.weight_range)
    ...
    def forward(self, x):
        x = torch.clamp(self.TAISP(x),1e-6,1) * 255.0 
        outs = []
